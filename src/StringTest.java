...
public class StringTest {
	...
	public String splitWords(String s) {
		...
		return new StringBuilder().append(word1).append(" ").append(word2)
				.toString();
	}
	...
	public Boolean hasUniqueChars(String s) {
		...
		for (int i = 0; i < length; i++) {
			if (charMap.containsKey(String.valueOf(chars[i]))) {
				System.out.println("String: " + String.valueOf(chars[i])
						+ " Result: " + chars[i]);
	...
	public Boolean isUnique3(String s) {
		...
		for (int i = 0; i < s.length(); i++) {
			...
			System.out.println("Before :char: " + s.charAt(i) + " value: "
					+ val + " checker: " + checker);
			if ((checker & (1 << val)) > 0) {
				System.out.println("Exit char: " + s.charAt(i) + " value: "
						+ val + " checker: " + checker);
				return Boolean.FALSE;
			}

			checker |= (1 << val);
			System.out.println("char: " + s.charAt(i) + " value: " + val
					+ " checker: " + checker);
		}
		return Boolean.TRUE;
	}
	...
	public void test1() {
		...
		// assert isUnique3(s1);
		/*
		 * Assert.AssertTrue(); String s2 = “ABCDEFGHIJKLMNOPQRSTUVWXYZABC”;
		...
		for (int i = 0; i < n; i++) {
			for (int j = 0; j < m; j++) {
				...
				if (pattern[j] == GAP) {
					k = skipper;
					j++;
					while (k < n && pattern[j] != text[i + j + k])
						k++;
					skipper = k - (i + j);
					continue;
	...
	public static boolean isPermutationOf2(String s1, String s2) {
	...
	public static boolean isPermutationOf(String s1, String s2) {
		...
		for (char s : s1_arr) {
			int c = counts.containsKey(s) ? counts.get(s) : 0;
			counts.put(s, c++);
		}
	...
	public static void main(String args[]) {
		...
		System.out.println(st
				.isPermutationOf2("aaaabbbbcccc", "aaaabbbbccccc"));
